var StepBoard = function (a, b) {
	...
	this.stepboardlabel = new createjs.Text(d.toString(), "bold 60px Arial","yellow", b);
...
function LoadWelComeSceneRes() {
	queue.loadManifest({path:RES_DIR + "img/", manifest:[{src:"banner.jpg", id:"banner"}, {src:"help.jpg", id:"help"}, {src:"discription.png", id:"discription"}, {src:"playbtn.png", id:"playbtn"},{src:"playagainbtn.png",id:"playagainbtn"} ,{src:"topbtn.png", id:"topbtn"}]}, !1);
}
...
function LoadGameoverSceneRes() {
	queue.loadManifest({path:RES_DIR + "img/", manifest:[{src:"gameoverbg.png", id:"gameoverbg"},{src:"sharebtn.png", id:"sharebtn"},{src:"shareframe.png",id:"shareframe"},{src:"comment1.png",id:"comment1"},{src:"comment2.png",id:"comment2"},{src:"comment3.png",id:"comment3"},{src:"comment4.png",id:"comment4"},{src:"comment5.png",id:"comment5"}]}, !1);
}
function GameLevel(a,b,c,d){
    $.ajax({
        url: '/hd/msklm',
        type: 'post',
        data: {
        ...
        if (data.code === 0 && !gameover) {
            gamelevel = data.percentage;
            new GameoverNormal(a,b,c,d,gamelevel);
        }
        else if (data.code === 2) {
            alert("参数错误");
        }
...
var GameoverNormal = function (a, b, c, d,f) {
    gameover = true;
	...
	e.addChild(a);
	a = new createjs.Bitmap(queue.getResult("shareframe"));
	a.setAnchorPoint(0.5, 0.5);
	a.x = 320;
	a.y = 390;
	e.addChild(a);
    a = Math.abs(score);
    a = "我用了"+ a +"步完成了马赛克连盟,"
	a = new createjs.Text(a.toString(), "30px Arial","white", b);
    ...
    a = Math.abs(score);
    a += "步";
    a = new createjs.Text(a.toString(), "60px Arial","yellow", b);
    a.textBaseline = "middle";
    a.x = 280;
    a.y = 460;
    e.addChild(a);
	...
	b.y = 420;
	e.addChild(b);*/
    if(90<=gamelevel && gamelevel <100)
	{
        b = new createjs.Bitmap(queue.getResult("comment5"));
    }
    else if(80<=gamelevel && gamelevel <90)
	{
        b = new createjs.Bitmap(queue.getResult("comment4"));
    }
    else if(60<=gamelevel && gamelevel <80)
	{
        b = new createjs.Bitmap(queue.getResult("comment3"));
    }
    else if(40<=gamelevel && gamelevel <60)
	{
        b = new createjs.Bitmap(queue.getResult("comment2"));
    }
    else if(gamelevel <40)
	{
        b = new createjs.Bitmap(queue.getResult("comment1"));
    }
	b.setAnchorPoint(0.5, 0.5);
	b.x = 340;
	b.y = 540;
	e.addChild(b);
	b = new createjs.Bitmap(queue.getResult("playagainbtn"));
	b.setAnchorPoint(0.5, 0.5);
	b.x = 179;
	b.y = 730;
	e.addChild(b);
...
}, qp_a = 12, qp_b = 14, qp_c = 25, qp_d = 27, qp_e = 80, qp_f = 50, qp_g = 0, qp_h = 0, COLOR_RED = "#ef4653", COLOR_GREEN = "#32859f", COLOR_PURPLE = "#f9d7a9", COLOR_YELLOW = "#f7b35c", COLOR_BLUE = "#97c1cd", COLOR_PINK = "#f8a2a9", qp_i, qp_j, qp_k = null;
...
function qp_m() {
	...
	qp_j.addChild(a);
	a.on("mousedown", function (a) {
		IS_TOUCH && a.nativeEvent instanceof MouseEvent || (this.scaleY = this.scaleX = 0.95, qp_k.flood(COLOR_RED), !0 == qp_k.isGameOver() && new GameLevel("#8de9cb", "#000000", qp_m, stage));
	}, a);
	a.on("pressup", function (a) {
		this.scaleY = this.scaleX = 1;
	}, a);
	...
	qp_j.addChild(a);
	a.on("mousedown", function (a) {
		IS_TOUCH && a.nativeEvent instanceof MouseEvent || (this.scaleY = this.scaleX = 0.95, qp_k.flood(COLOR_YELLOW), !0 == qp_k.isGameOver() && new GameLevel("#8de9cb", "#000000", qp_m, stage));
	}, a);
	a.on("pressup", function (a) {
		this.scaleY = this.scaleX = 1;
	}, a);
	...
	qp_j.addChild(a);
	a.on("mousedown", function (a) {
		IS_TOUCH && a.nativeEvent instanceof MouseEvent || (this.scaleY = this.scaleX = 0.95, qp_k.flood(COLOR_PURPLE), !0 == qp_k.isGameOver() && new GameLevel("#8de9cb", "#000000", qp_m, stage));
	}, a);
	a.on("pressup", function (a) {
		this.scaleY = this.scaleX = 1;
	}, a);
	...
	qp_j.addChild(a);
	a.on("mousedown", function (a) {
		IS_TOUCH && a.nativeEvent instanceof MouseEvent || (this.scaleY = this.scaleX = 0.95, qp_k.flood(COLOR_BLUE), !0 == qp_k.isGameOver() && new GameLevel("#8de9cb", "#000000", qp_m, stage));
	}, a);
	a.on("pressup", function (a) {
		this.scaleY = this.scaleX = 1;
	}, a);
	...
	qp_j.addChild(a);
	a.on("mousedown", function (a) {
		IS_TOUCH && a.nativeEvent instanceof MouseEvent || (this.scaleY = this.scaleX = 0.95, qp_k.flood(COLOR_GREEN), !0 == qp_k.isGameOver() && new GameLevel("#8de9cb", "#000000", qp_m, stage));
	}, a);
	a.on("pressup", function (a) {
		this.scaleY = this.scaleX = 1;
	}, a);
	...
	qp_j.addChild(a);
	a.on("mousedown", function (a) {
		IS_TOUCH && a.nativeEvent instanceof MouseEvent || (this.scaleY = this.scaleX = 0.95, qp_k.flood(COLOR_PINK), !0 == qp_k.isGameOver() && new GameLevel("#8de9cb", "#000000", qp_m, stage));
	}, a);
	a.on("pressup", function (a) {
		this.scaleY = this.scaleX = 1;
	}, a);
...
Qp_n.prototype.flood = function (a, b) {
    ...
    if(this.m_fxarray.length >= 0 && !qp_k.isGameOver())
    {
        qp_q(this.m_fxarray, a, b), qp_i.setStepNum_IncreaseOneStep();
    }
	this.m_stepover = !1;
};
